...
const HeroSection = () => {
  ...
  return (
    <section id="inicio" className="relative min-h-[100vh] overflow-hidden flex items-center bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="absolute inset-0 z-1 opacity-10">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.1)_1px,transparent_1px)] bg-[size:50px_50px] [mask-image:radial-gradient(ellipse_at_center,transparent_20%,black_70%)]" />
      </div>
      ...
      <div className="relative z-10 max-w-container mx-auto px-4 md:px-8 h-full flex flex-col justify-center">
        ...
        >
          ...
          <motion.h1 
            className="text-4xl md:text-6xl lg:text-7xl text-white mb-6 leading-tight font-light"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.4 }}
          >
            <span className="block">{t('hero.title_line1', 'Há mais de 80 anos')}</span>
            <span className="block font-bold bg-gradient-to-r from-white to-blue-200 bg-clip-text text-transparent">{t('hero.title_line2', 'bombeando soluções.')}</span>
          </motion.h1>
          ...
          <motion.div 
            className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6 mt-8 md:mt-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
